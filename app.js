...
const userIcon = L.divIcon({
  ...
  html: `<svg viewBox="0 0 100 100">
    <path fill="#2196f3" d="M50 5 L88 95 L50 75 L12 95 Z"/>
  </svg>`,
  iconSize: [24, 24],
  iconAnchor: [12, 12]
});
  ...
  if (!following) {
    ...
    watchId = navigator.geolocation.watchPosition(
      ...
      (pos) => {
        ...
        if (altitude !== null) {
          altitudeLabel.innerText = '標高 ' + Math.round(altitude) + ' m';
        }
      ...
      (err) => {
        gpsBtn.classList.remove('following');
...
compassBtn.addEventListener('click', async () => {
  ...
  if (compassEnabled) {
    ...
    compassBtn.classList.remove('compass-on');
    return;
  }
  ...
  try {
    ...
    compassEnabled = true;
    compassBtn.classList.add('compass-on');

  } catch (e) {
    compassBtn.classList.add('error');
    alert('コンパス取得失敗');
  }
